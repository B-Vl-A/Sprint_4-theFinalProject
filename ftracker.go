...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	a := float64(runningCaloriesMeanSpeedMultiplier) //18
	b := runningCaloriesMeanSpeedShift               //1.79
	c := meanSpeed(action, duration)
	spentCalories := ((a * c * b) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	a := walkingCaloriesWeightMultiplier         //0.035
	b := walkingSpeedHeightMultiplier            //0.029
	c := meanSpeed(action, duration) * kmhInMsec //м/с
	d := height / cmInM                          // рост в метрах
	spentCalories := ((a*weight + (math.Pow(c, 2)/d)*b*weight) * duration * minInH)
	return spentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	a := swimmingCaloriesMeanSpeedShift                     //1.1
	b := float64(swimmingCaloriesWeightMultiplier)          //2
	c := swimmingMeanSpeed(lengthPool, countPool, duration) //км/ч
	spentCalories := (c + a) * b * weight * duration
	return spentCalories
}
